...
//     const float looseElCuts[2][10] = {{0.016,0.08,0.012,0.15,0.019,0.036,0.11,0.18,1e-6,1},{0.025,0.097,0.032,0.12,0.099,0.88,0.11,0.21,1e-6,1}};    
    const float looseElCuts[2][10] = {{0.021,0.25,0.012,0.24,0.031,0.5,0.32,0.24,1e-6,2},{0.028,0.23,0.035,0.19,0.22,0.91,0.13,0.24,1e-6,3}}; 
...
void passTightMuonID(TMuon* muon, vector<bool> &passCuts ) {
    ...
    if( ((muon->chHadIso04 + max(muon->gammaIso04+muon->neuHadIso04-0.5* muon->puIso04,0.0))/muon->pt) < 0.2 ) passCuts.push_back(true);
    else passCuts.push_back(false);
}
